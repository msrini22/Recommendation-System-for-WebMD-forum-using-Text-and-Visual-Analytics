function createVisualization(topic) {
    document.getElementById("metric-modal").innerHTML = "";
    // For word cloud
    $('#imageToSwap').attr('src', './images/'+topic+".png");

    // For line Chart
    d3.json("./combinetopics.json", function(data){

        var metricCount = [];
        var metricMonths = [];

        var metricData = data[topic];
        
        for(var i = 0; i < metricData.length; i++)  {
            metricCount.push(metricData[i].count);
            metricMonths.push(metricData[i].topic);
        }

        console.log(metricMonths);
        ...
        */
        ...
        if (dateRange < ms_in_year)   {
            ...
            var button_data =["Year","Month","Reset"];
        };
